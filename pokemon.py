from requests import api as Req
...
def nome_do_pokemon(numero):
    if numero <= 0 and numero >= 5000:
        ...
        url = f"http://localhost:8080/api/v2/pokemon/{numero}"
        pokemon = Req.get(url).json()
        return pokemon['name']
...
def nivel_do_pokemon(nome, experiencia):
    ...
    url = f"http://localhost:8000/api/v2/pokemon-species/{nome}"
    pokemon = api.get(url).json()
    if "name" in pokemon:
        url_nivel = pokemon["growth_rate"]["url"]
        lista_xp = []
        nivel = api.get(url_nivel).json()
        lista_nivel = nivel["levels"]

        for i in range(len(lista_nivel)):
            lista_xp.append(lista_nivel[i]["experience"])

            if experiencia in lista_xp:
                if lista_nivel[i]["experience"] == experiencia:
                    level = lista_nivel[i]["level"]

            else:
                menor_xp = max(i for i in lista_xp if i < experiencia)
                if lista_nivel[i]["experience"] == menor_xp:
                    level = lista_nivel[i]["level"]
